...
export function ContactSection({ settings }: { settings: PortfolioSettings | null }) {
  ...
  const email = settings?.email ? t.getMultilingualText(settings.email) : CONTACT_EMAIL
  const github = settings?.github ? t.getMultilingualText(settings.github) : GITHUB_URL
  const linkedin = settings?.linkedin ? t.getMultilingualText(settings.linkedin) : LINKEDIN_URL
