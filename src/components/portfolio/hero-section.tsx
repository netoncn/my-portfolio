...
import { fadeInUp, slideInFromLeft, slideInFromRight, staggerContainer, smooth } from "@/lib/animations"
...
export function HeroSection({ settings }: { settings: PortfolioSettings | null }) {
  ...
  return (
    <section className="relative min-h-[90vh] flex items-center justify-center px-4 py-20">
      <motion.div className="container" variants={staggerContainer} initial="initial" animate="animate">
        <motion.div className="space-y-4 mb-12" variants={slideInFromLeft} transition={smooth as any}>
          <h1 className="text-5xl md:text-7xl font-bold tracking-tight text-balance">{settings?.name || "Portfolio"}</h1>
          <p className="text-xl md:text-2xl text-muted-foreground font-light">{t.getMultilingualText(settings?.role) || "Web Developer"}</p>
        </motion.div>
        ...
        <div className="max-w-7xl w-full flex flex-col md:flex-row gap-12 items-start md:items-center mx-auto">
          ...
          <div className="w-full md:w-auto flex flex-col items-center gap-8 order-1 md:order-2">
            ...
            {settings && (
              <motion.div className="flex gap-4" variants={fadeInUp} transition={{ ...smooth as any, delay: 0.3 }}>
                {settings.github && (
